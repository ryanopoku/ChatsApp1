 ...
 Default Implementation:
    ...
    if ([[C2CallPhone currentPhone] hasObjectForKey:self.richMessageKey]) {
        ...
        if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]) {
            [cv addChoiceWithName:NSLocalizedString(@"Save", @"Choice Title") andSubTitle:NSLocalizedString(@"Save to your Camera Roll", @"Button") andIcon:[UIImage imageNamed:@"ico_image.png"] andCompletion:^{
                [self saveToAlbum:nil];
            }];
        }
    } else {
        [cv addChoiceWithName:NSLocalizedString(@"Download", @"Choice Title") andSubTitle:NSLocalizedString(@"Download from Server", @"Button") andIcon:[UIImage imageNamed:@"ico_download.png"] andCompletion:^{
